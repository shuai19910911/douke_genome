...
训练资源策略：优先让正式方案能在 2 张 A100 40G GPU 范围内启动和推进；如果正式 token budget、上下文长度或吞吐要求超出该配置，则扩展到更多 GPU，而不是降低为非正式小模型。当前方案已经调整为结构注释驱动路线：正式训练只使用有结构注释的 251 个基因组。
...
如果在本服务器完成数据处理，再搬到其他服务器训练，推荐准备 **1.5-2.5 TB** 的训练数据包；最低可训练包约 **0.8-1.2 TB**，完整处理缓存约 **3-5 TB**。目标训练服务器建议至少预留 **4 TB** 可用空间，完整缓存和多 checkpoint 场景建议 **6-8 TB**。
