...
def test_similarity_aggregation_clusters_same_binomial_near_duplicates(tmp_path: Path) -> None:
    data = tmp_path / "raw"
    data.mkdir()
    random.seed(7)
    ...
    assert by_id["a" * 16]["near_duplicate_group_size"] == 2
    assert by_id["b" * 16]["near_duplicate_group_id"] == by_id["a" * 16]["near_duplicate_group_id"]
    assert by_id["c" * 16]["near_duplicate_group_size"] == 1
