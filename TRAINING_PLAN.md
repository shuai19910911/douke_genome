...
> 文档状态：**候选合同（尚不可启动正式训练）**，最后更新：2026-07-22。固定上下文集合为1K、8K、32K、64K、128K和256K；74-source数据候选已闭合，参数设计公式为314,669,504，cold-genus/schema-2、H20运行时参数审计和总token预算仍在冻结门禁内。详细数据、模型、任务和基线方案见`LEGUMEGENOMEFM_RESEARCH_DESIGN.md`。任何`superseded`（已废弃）短上下文配置均不得启动。
...
最终过滤后重新构建split，旧六个cold genus不自动继承。基于74-source容量审计，当前**提案**为Phaseolus作为cold-development、Arachis与Vicia作为sealed cold test；三属均不得进入预训练，直到schema-2 receipt闭合前仍标记`PROPOSED_NOT_FROZEN`。至少维护四层报告：
...
机器合同为`configs/evaluation_matrix.yaml`，当前状态是`proposed_cold_genus_and_specialist_tasks_not_frozen`。完整逐任务输入、来源、长度、指标和baseline见`LEGUMEGENOMEFM_RESEARCH_DESIGN.md`。核心任务覆盖至少六个层级：
...
核心比较固定为AgroNT-1B、PlantCAD2 Small/Medium/Large、Nucleotide Transformer v2 500M multi-species和Evo-2-7B；Evo-2-40B仅在2–3×H20真实运行gate通过后加入。补充比较GPN、PlantCaduceus、Caduceus和HyenaDNA；PlantGFM仅在公开权重、许可证、输入语义和推理代码核验后进入主表。每个基线使用正式tokenizer和推荐输入；长任务同时报告共同1K/8K、native-full和tile-pooling，不能把短模型“原生长度不合格”计成0分。
