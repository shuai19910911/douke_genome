...
> 状态：**CANDIDATE / PROFILING_REQUIRED（唯一候选，尚未冻结）**，最后更新：2026-07-20。正式最大上下文候选为262,144 bp；模型已建立代码与静态合同，但目标环境尚无`mamba-ssm`生产kernel，也没有H20实测receipt，因此精确参数量、峰值显存、吞吐和正式训练预算不得填写为已验证值。
...
候选配置已固定模块形状，但当前**精确可训练参数量未冻结**，原因是生产Mamba-2依赖尚未安装，不能用手算或假module冒充真实`named_parameters()`结果。
...
7. 三者hash一致后将`parameter_count`从字符串改为整数。
